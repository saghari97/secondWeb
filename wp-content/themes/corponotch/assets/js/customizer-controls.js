...
jQuery( document ).ready(function($) {
    ...
    $(".corponotch-chosen-select").chosen({
        width: "100%"
    });

    // icon picker
    $('.corponotch-icon-picker').each( function() {
    ...
    $( document ).on( 'click', '.customize_multi_add_field', corponotch_customize_multi_add_field )
        .on( 'change', '.customize_multi_single_field', corponotch_customize_multi_single_field )
        .on( 'click', '.customize_multi_remove_field', corponotch_customize_multi_remove_field )
    ...
    function corponotch_customize_multi_add_field(e) {
        ...
        e.preventDefault();
            ...
            corponotch_customize_multi_write( $control );
    }

    function corponotch_customize_multi_single_field() {
        var $control = $( this ).parents( '.customize_multi_input' );
        corponotch_customize_multi_write( $control );
    }

    function corponotch_customize_multi_remove_field(e) {
        e.preventDefault();
        var $this = $( this );
        var $control = $this.parents( '.customize_multi_input' );
        $this.parent().remove();
    ...
    function corponotch_customize_multi_write( $element) {
...
jQuery(document).ready(function($) {
    // Sortable sections
    jQuery( 'ul.corponotch-sortable-list' ).sortable({
        handle: '.corponotch-drag-handle',
        axis: 'y',
        update: function( e, ui ){
            jQuery('input.corponotch-sortable-input').trigger( 'change' );
        }
    });

    /* On changing the value. */
    jQuery( "body" ).on( 'change', 'input.corponotch-sortable-input', function() {
        /* Get the value, and convert to string. */
        this_checkboxes_values = jQuery( this ).parents( 'ul.corponotch-sortable-list' ).find( 'input.corponotch-sortable-input' ).map( function() {
            return this.value;
        }).get().join( ',' );

        /* Add the value to hidden input. */
        jQuery( this ).parents( 'ul.corponotch-sortable-list' ).find( 'input.corponotch-sortable-value' ).val( this_checkboxes_values ).trigger( 'change' );
